...
provide('todaySignedIn', todaySignedIn)
...
const connectWebSocket = () => {
  ...
  stompClient.onConnect = (frame) => {
    ...
    stompClient.subscribe('/topic/drawing_complete', (message) => {
      ...
      // 通知个人中心有新图片生成
      window.dispatchEvent(
        new CustomEvent('drawingCompleted', {
...
<template>
  <el-container class="main-container">
      ...
      <div class="header-content">
        <div class="logo">
          ...
          <span
            ...
            :title="wsConnected ? 'WebSocket 已连接' : 'WebSocket 断开, 正在重连...'"
          ></span>
        </div>
        <el-menu
          :default-active="activeView"
          mode="horizontal"
          @select="(index) => navigateTo(index)"
          :ellipsis="false"
        >
    ...
    <!-- 登录对话框组件 (无变动) -->
    <!-- 认证对话框 -->
    <AuthDialog v-model="authDialogVisible" @login-success="handleLoginSuccess" />
  </el-container>
</template>
